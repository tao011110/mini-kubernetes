...
|2|实现解析yaml的函数接口, 并判断输入的配置文件是否完整正确|lsh|待定|
|3|实现单个docker容器创建和销毁的函数接口|tyc hjk|/|
|4|实现pod抽象, 实现对pod中各容器的整体创建(创建前需要检查本地资源是否满足request)|tyc hjk|/|
|5|实现对多个Pod的IP分配, 使同一Pod内各容器可以通过localhost相互访问, 并严格保证多机并行下的全局唯一IP分配|tyc|/|
|6|实现对Pod的探测接口|tyc|/|
|7|实现kubelet, 对本机Pod进行探测和重启|hjk|/|
|8|实现etcd, 实现分布式存储功能|lsh|/|
|9|实现kubectl, 接收解析和转发用户指令|lsh|/|
|10|实现master|tyc|/|
|11|实现master和kuberctl之间的通信|hjk|/|
|12|测试|全体|/|
